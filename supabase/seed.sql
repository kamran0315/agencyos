...
begin
  ...
  delete from public.tasks where project_id in (select id from public.projects where owner_id = owner);
  ...
  insert into public.notes (owner_id, project_id, client_id, title, body, category, pinned) values
    (owner, p_brightline, c_sarah,  'Brand voice guidelines',                 'Warm, confident, never corporate. Avoid em-dashes. Sentence case headings.',                                                                  'requirement', true),
    (owner, p_investor,   c_daniel, 'Hosting credentials (Vercel + Supabase)','Vercel team: park-capital. DB: prod-supa-east. Rotate secrets quarterly.',                                                                   'credential',  true),
    (owner, p_investor,   c_daniel, 'Kickoff call notes — Apr 12',            'Daniel wants the v2 portal in time for Q2 LP meeting. Key deliverables: SSO, quarterly reports, doc vault.',                                  'meeting',     false),
    (owner, p_brightline, c_sarah,  'Revision round 2 — Sarah',               'Hero CTA copy is too pushy. Try "See what we built" instead. Reduce shadow on cards.',                                                        'revision',    false),
    (owner, null,         null,     'Internal — Q3 hiring',                   'Looking for a part-time React dev to take on smaller retainers.',                                                                              'internal',    false);
